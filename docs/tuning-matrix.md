...
By default, RAM tiers are auto-selected from host memory; explicit values in `bootstrap.env` take precedence.
...
`FORCE_RAM_TIER=S|M|L|XL` can force a specific tier.
